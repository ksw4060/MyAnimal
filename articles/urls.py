...
urlpatterns = [
    ...
    path('<int:user_id>/hearts/',views.HeartsView.as_view(), name='User_Hearts_View'), # 좋아요 한 게시글
  
    path('<int:article_id>/bookmarks/',views.BookMarksView.as_view(), name='bookmarks_View'),  # 북마크 기능
    path('<int:user_id>/bookmarks/',views.BookMarksView.as_view(), name='bookmarks_View'),  # 북마크 한 게시글
